...
When the plugin detects that it runs embedded in Nextcloud (i.e. the top window is owned by Nextcloud), it changes Roundcube's behaviour slightly:

- some styles are overwritten from Nextcloud's styles.
- when a session error occurs (e.g. timeout), Roundcube will not redirect to its own login page. Instead, the top window is reloaded, which triggers the Roundcube Mail App to re-authenticate its sibling.
